...
func main() {
	...
	for _, r := range rows {
		strValues := strings.Split(r, "\t")
		q := findHiddenQuotient(strValues)
		otherChecksum += q
	}
	fmt.Println(otherChecksum)
}

func strMinxMax(strValues []string) (int, int) {
	min := 10000000
	max := 0
	for _, strV := range strValues {
...
func findHiddenQuotient(strValues []string) int {
	intValues := []int{}
	for _, sv := range strValues {
		intV, _ := strconv.Atoi(sv)
		intValues = append(intValues, intV)
	}

	for i, someInt := range intValues {
		for j, someOtherInt := range intValues {
			if i == j {
				continue
			}
			if someInt%someOtherInt == 0 {
				return someInt / someOtherInt
			}
		}
	}
	log.Printf("No divisors found for %+v", intValues)
	return -1
}
